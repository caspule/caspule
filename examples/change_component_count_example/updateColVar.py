...
def fixAtomNumbers(n, NA, NB, NC, seg):
    """
    ...
    numAtoms = chainLen * (int(NA) + int(NB) + int(NC))
    indices = []
    step = chainLen // 2
...
def updateCV(cvfile, Rg, L, n, NA, NB, NC, seg):
    """
    ...
    for i, line in enumerate(lines):
        ...
        if re.search('atomNumbers', line):
            lines[i] = fixAtomNumbers(n, NA, NB, NC, seg)
...
if __name__ == "__main__":
    _, xyzFile, cvfile, L, n, NA, NB, NC, seg = sys.argv

    posArr = parseXYZfile(xyzFile)
    com, Rg = calc_RadGy(posArr)
    updateCV(cvfile, Rg, L, n, NA, NB, NC, seg)
