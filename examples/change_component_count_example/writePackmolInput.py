...
def generate_packmol_input(n, NA, NB, NC, L, inp_file, outfile):
    """
    ...
    with open(f'{inp_file}', 'w') as tf:
        ...
        tf.write(f'structure polyC_n{n}_mono.xyz\n')
        tf.write(f'\tnumber {NC}\n')
        tf.write(f'\tinside box  {-L}  {-L} {-L}  {L} {L} {L}\n')
        tf.write('end structure\n')


if __name__ == "__main__":
    _, n, NA, NB, NC, L, inp_file, outfile = sys.argv 
    n, NA, NB, NC, L = int(n), int(NA), int(NB), int(NC), float(L)
    generate_packmol_input(n, NA, NB, NC, L, inp_file, outfile)
